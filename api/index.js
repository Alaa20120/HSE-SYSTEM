...
app.use('/api', (req, res, next) => {
  ...
  if (cleanPath === '/auth/login' || cleanPath === '/auth/seed') {
    return next();
  }
...
const baseUploadsDir = isVercel ? '/tmp' : path.join(__dirname, '..');
...
app.delete('/api/ptws/:id', async (req, res) => {
  try {
    ...
    if (ptw.filePath) deletePhysicalFile(ptw.filePath);
    if (Array.isArray(ptw.photos)) ptw.photos.forEach(photo => deletePhysicalFile(photo.path));
    if (Array.isArray(ptw.reports)) ptw.reports.forEach(report => deletePhysicalFile(report.path));
...
app.post('/api/near-misses', async (req, res) => {
  try {
    ...
    const newNearMiss = await LocalDB.insert('near_misses', {
      ...
      site,
      description,
      potentialSeverity: potentialSeverity || 'Low',
